        ...
        {news.length > 0 &&
          news.map((item, index) => (
            <SwiperSlide key={index}>
              <div className="flex flex-col sm:flex-row items-center  sm:justify-between gap-12 bg-background border rounded-md p-4 border-black">
                <div className="py4">
                  <Link to="/">
                    <h3 className="text-lg font-medium hover:text-orange-500 mb-4">
                      {item.title}
                    </h3>
                  </Link>
                  <div className="w-10 h-[4px] bg-secondary"></div>
